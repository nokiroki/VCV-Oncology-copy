...
class Conv2dAutoEncoder(pl.LightningModule):
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=512, kernel_size=3, stride=2),
            nn.ReLU(),
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=self.out, kernel_size=3),
            nn.ReLU()
        )
        self.encoder.apply(init_weights)
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.out, out_channels=32, kernel_size=3),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            ...
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=512, out_channels=in_channels, kernel_size=3, stride=2),
        )
    ...
    def forward(self, x):
        ...
        latent = torch.mean(encoded_img, (2, 3))
